#!/usr/bin/env python3
"""
run_optuna_mlp_simple.py
========================
Optuna hyperparameter search for MLPSimpleModel (ablation variant 1).
...
from src.models.mlp_ablation_models import MLPSimpleModel
import torch, yaml
from sklearn.metrics import matthews_corrcoef

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
                    handlers=[logging.StreamHandler(sys.stdout)])
log = logging.getLogger("optuna_mlp_simple")
...
log.info(f"MLPSimple Optuna | features={len(feat_cols)} | {len(search_folds)} folds | 50 trials")
...
log.info(f"\nRESULTADO MLPSimple (7 folds):  Acc={summary['accuracy_mean']:.4f}  MCC={summary['mcc_mean']:.4f}  AUC={summary['roc_auc_mean']:.4f}")
log.info(f"RESULTADO MLPSimple (folds 1-6, 2019-2024):  Acc={summary_stable['accuracy_mean']:.4f}  MCC={summary_stable['mcc_mean']:.4f}  AUC={summary_stable['roc_auc_mean']:.4f}")

out = {
    "experiment_id"  : "MLPSimple_G3_Optuna",
...
with open(results_dir / "mlp_simple_optuna.json", "w") as f:
    json.dump(out, f, indent=2)
study.trials_dataframe().to_csv(results_dir / "optuna_mlp_simple_50trials.csv", index=False)
