#!/usr/bin/env python3
"""
run_optuna_mlp_dual_noattn.py
==============================
Optuna hyperparameter search for MLPDualNoAttnModel (ablation variant 2).

Identical config to run_optuna_mlp.py (MLP Dual V2):
  50 trials, TPE sampler, folds 1-6 para busqueda, dataset original 33 features.

Search space adaptado: sin parametros de atencion (n_context_tokens, n_heads, n_cross_layers).
Tiene encoder dual (dos ramas separadas) pero fusion por concatenacion simple, sin cross-attention.
"""
...
from src.models.mlp_ablation_models import MLPDualNoAttnModel
import torch, yaml
from sklearn.metrics import matthews_corrcoef

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
                    handlers=[logging.StreamHandler(sys.stdout)])
log = logging.getLogger("optuna_mlp_dual_noattn")
...
log.info(f"MLPDualNoAttn Optuna | tech={len(tech_cols)} onchain={len(onchain_cols)} | {len(search_folds)} folds | 50 trials")
...
def objective(trial: optuna.Trial) -> float:
    cfg = {
        ...
        "n_encoder_layers": trial.suggest_int("n_encoder_layers", 2, 5),
...
log.info(f"\nRESULTADO MLPDualNoAttn (7 folds):  Acc={summary['accuracy_mean']:.4f}  MCC={summary['mcc_mean']:.4f}  AUC={summary['roc_auc_mean']:.4f}")
log.info(f"RESULTADO MLPDualNoAttn (folds 1-6, 2019-2024):  Acc={summary_stable['accuracy_mean']:.4f}  MCC={summary_stable['mcc_mean']:.4f}  AUC={summary_stable['roc_auc_mean']:.4f}")

out = {
    "experiment_id"  : "MLPDualNoAttn_G3_Optuna",
...
with open(results_dir / "mlp_dual_noattn_optuna.json", "w") as f:
    json.dump(out, f, indent=2)
study.trials_dataframe().to_csv(results_dir / "optuna_mlp_dual_noattn_50trials.csv", index=False)
