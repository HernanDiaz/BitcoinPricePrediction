#!/usr/bin/env python3
"""
run_optuna_lgbm.py — Optuna para LightGBM G3.
Configuracion identica a run_optuna_xgb.py y run_optuna_mlp.py:
...
log.info(f"LightGBM Optuna | {len(feat_cols)} features | {len(search_folds)} search folds | 50 trials")
...
def objective(trial):
    ...
    for fold in search_folds:
        ...
        clf = lgb.LGBMClassifier(scale_pos_weight=spw, **params)
        clf.fit(X_tr, y_tr)
...
for fold in cv:
    ...
    params.update({"scale_pos_weight": spw, "random_state": seed, "verbosity": -1, "n_jobs": -1})
...
log.info(f"\nRESULTADO LightGBM (7 folds):  Acc={summary['accuracy_mean']:.4f}  MCC={summary['mcc_mean']:.4f}  AUC={summary['roc_auc_mean']:.4f}")
...
out = {"experiment_id": "LightGBM_G3_Optuna_final", "dataset": "original_33features",
...
with open(results_dir / "lightgbm_g3_optuna_final.json", "w") as f:
    json.dump(out, f, indent=2)
study.trials_dataframe().to_csv(results_dir / "optuna_lgbm_50trials.csv", index=False)
