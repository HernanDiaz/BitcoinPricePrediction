...
run_optuna_xgb.py
=================
XGBoost exhaustive Optuna optimisation — 300 trials, wider search space,
hybrid TPE (50 trials) -> CMA-ES (250 trials) sampler.
"""
...
# Usar solo folds 1-6 (2019-2024) para la búsqueda — fold 7 es régimen diferente
search_folds = list(cv)[:-1]

results_dir = ROOT / "results" / "optuna"
results_dir.mkdir(parents=True, exist_ok=True)

log.info(f"XGBoost Optuna exhaustivo | {len(feat_cols)} features | {len(search_folds)} folds (2019-2024) | 300 trials")
...
def objective(trial: optuna.Trial) -> float:
    ...
    for fold in search_folds:
        ...
        clf = xgb.XGBClassifier(scale_pos_weight=neg/max(pos,1), **params)
        clf.fit(X_tr, y_tr)
...
# Sampler hibrido: TPE (50) -> CMA-ES (250)
...
# Cambiar a CMA-ES con el mejor punto como inicio
...
# Evaluacion final en los 7 folds
log.info("\nEvaluacion final (7 folds) con mejores params ...")
import xgboost as xgb
...
# Media excluyendo fold 7 (regimen diferente)
summary_stable = aggregate_fold_metrics(fold_metrics[:-1])
log.info(f"\nRESULTADO XGBoost_G3_Optuna_v2 (7 folds):")
...
out = {
    "experiment_id"  : "XGBoost_G3_Optuna_v2",
    "n_trials"       : 300,
...
with open(results_dir / "xgboost_g3_optuna_v2.json", "w") as f:
    json.dump(out, f, indent=2)

study.trials_dataframe().to_csv(results_dir / "optuna_xgb_300trials.csv", index=False)
